    ...
    <main class="app-main">
        <div class="app-content-header">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-6">
                        <h3 class="mb-0">Sub Menu Management</h3>
                    </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-md-12">
                        <div class="card card-primary card-outline mb-4">
                            <div class="card-header">
                                <div class="card-title">Add Sub Menu</div>
                            </div>
                            <div class="card-body">
                                <form action="{{ route('submenu_add') }}" method="POST">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" name="submenu_url"
                                ...
                                <table id="submenuTablee" class="table table-bordered">
                                    ...
                                    <tbody>
                                        @php
                                            ...
                                            // Pre-count rowspans for each unique menu name
                                            foreach ($submenudata as $submenu) {
                                                $menuName = $submenu->menu_name;
                                                if (!isset($rowspanCounts[$menuName])) {
                                                    $rowspanCounts[$menuName] = $submenudata
                                                        ->filter(function ($item) use ($menuName) {
                                            ...
                                            $renderedMenus = []; // To track already rendered menu names
                                        @endphp
                                        ...
                                        @forelse ($submenudata as $index => $submenu)
                                            <tr>
                                                ...
                                                @if (!in_array($submenu->menu_name, $renderedMenus))
                                                    <td rowspan="{{ $rowspanCounts[$submenu->menu_name] }}">
                                                        <strong>{{ $submenu->menu_name }}</strong>
                                                    </td>
                                                    @php $renderedMenus[] = $submenu->menu_name; @endphp
                                                @endif

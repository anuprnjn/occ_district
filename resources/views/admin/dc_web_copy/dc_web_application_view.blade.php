            ...
            <div class="container-fluid">
                <div class="row g-4">
                    <div class="col-md-12">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <table class="table table-bordered">
                                                ...
                                                <th class="fw-bold">Case No</th>
                                                    <td>{{ $dcuser->type_name }}/{{ $dcuser->case_number }}/{{ $dcuser->case_year }}
                                                    </td>
                                                @else
                                                    <th class="fw-bold">Filing No</th>
                                                                        <td>{{ $dcuser->type_name }}/{{ $dcuser->filing_number }}/{{ $dcuser->filing_year }}
                                                                        </td>
                                                @endif
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($order->upload_status)
                                                        ...
                                                        onclick="viewPDF(`{{ route('admin.downloadDcOrderCopy', ['fileName' => $order->file_name, 'date' => $order->certified_copy_uploaded_date]) }}`)">
                        ...
                        @if ($dcuser->document_status == 1 && $totaldiff > 0)
                            <div class="card card-success card-outline">
                                ...
                                <div class="card-body">
                                    ...
                                    @if ($dcuser->deficit_status == 1)
                                        <div class="alert mt-3">
                                            @if ($dcuser->deficit_payment_status == 0)
                                                <span class="alert alert-warning">Deficit Payment Pending.</span>
                                            @elseif ($dcuser->deficit_payment_status == 1)
                                    ...
                                    @else
                                        <form action="{{ route('dc-web-application.send-dc-deficit-notification') }}"
                                            method="POST">
                                            @csrf
                        ...
                        @elseif ($dcuser->document_status == 1 && $totaldiff == 0)
                            <form action="{{ route('dc-web-application.send-dc-ready-notification') }}" method="POST">
                                @csrf
                                <input type="hidden" name="application_number"
                                    value="{{ $dcuser->application_number }}">
                                <button type="submit" class="btn btn-primary mt-3">Send Certified Copy</button>
                            </form>
                        @elseif ($dcuser->document_status == 1 && ($dcuser->deficit_status == 1 && $dcuser->deficit_payment_status == 1))
                            <form action="{{ route('dc-web-application.send-dc-ready-notification') }}" method="POST">
                                @csrf
                                <input type="hidden" name="application_number"
                                    value="{{ $dcuser->application_number }}">
